...
export default function VoiceCard({ voice, onPlay }: VoiceCardProps) {
  return (
    ...
    rounded-lg transition-colors cursor-pointer group">
      {/* Voice Avatar - CLICKABLE ON MOBILE */}
      <div 
        onClick={(e) => {
          e.stopPropagation();
          onPlay?.();
        ...
        className="w-10 h-10 rounded-full bg-gradient-to-br
         from-[#43C6AC] to-[#191654]
          flex items-center justify-center text-white font-amiamie
           flex-shrink-0 uppercase cursor-pointer
           lg:cursor-default
      ...
      {/* Play Button - DESKTOP ONLY (hidden on mobile) */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onPlay?.();
        }}
        className="hidden lg:block text-xs text-black hover:text-blue-700 font-medium 
